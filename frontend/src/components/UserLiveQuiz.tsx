...
type UserLiveQuizProps = {
  currentQuestion: QuizQuestion;
  timeLeft: number;
  selectedOption: string | null;
  handleAnswerClick: (option: QuizOption) => void;
  totalTime?: number;
};
      ...
      <div className="w-full bg-white rounded-[2.5rem] shadow-2xl shadow-black/5 border border-black/5 flex flex-col overflow-hidden max-h-full min-h-0">
              ...
              <div className="flex items-center gap-2 bg-black/10 px-4 py-2 rounded-full backdrop-blur-md">
                <span
                  className={`w-2 h-2 rounded-full bg-white ${
                    isTimeCritical ? "animate-ping" : "animate-pulse"
                  }`}
                ></span>
                <span className="font-black text-xs text-black uppercase tracking-widest">
                  Live
              ...
              <div className="flex items-center gap-3 bg-white/90 px-4 py-1.5 rounded-2xl shadow-sm border border-black/5">
                ...
                <span
                  className={`font-mono font-black text-2xl tracking-tighter ${
                    isTimeCritical ? "text-red-500 animate-pulse" : "text-black"
                  }`}
                >
                  {timeLeft}s
                </span>
        ...
        {!isTimeUp && (
          <div className="shrink-0 w-full h-1.5 bg-[#72D177]/20 relative">
            <div
              className={`absolute top-0 left-0 h-full rounded-r-full transition-all duration-1000 ease-linear ${
                isTimeCritical ? "bg-red-500" : "bg-black"
              }`}
        ...
        <div className="shrink-0 px-6 pb-8 pt-4 md:px-12 md:pb-12 md:pt-6 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 w-full max-w-4xl mx-auto">
            {currentQuestion.options.map((option: QuizOption, i: number) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-xl font-black text-lg transition-colors ${
                      isSelected
                        ? "bg-black text-white"
                  ...
                  <span
                    className={`font-bold text-lg md:text-xl break-words pr-6 leading-snug ${
                      isSelected
                        ? "text-black"
                        : isDisabled
                        ? "text-gray-400"
