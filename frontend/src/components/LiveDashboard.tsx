...
export default function LiveDashboard({
  ...
  const progressPercentage =
    liveQuestions.length > 0
      ? ((currentQuestionIndex + 1) / liveQuestions.length) * 100
      : 0;

  const isLastQuestion =
    liveQuestions.length > 0 &&
    currentQuestionIndex === liveQuestions.length - 1;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-5xl mx-auto w-full">
            ...
            {currentQuestion?.options.map(
              (opt: QuizOption, i: number) => (
                ...
                >
                  <div
                    className={`w-10 h-10 md:w-12 md:h-12 shrink-0 flex items-center justify-center rounded-xl font-black text-lg mr-4 ${
                      opt.isCorrect
                        ? "bg-[#72D177] text-white shadow-md"
                  ...
                  <span
                    className={`font-bold text-base md:text-lg flex-1 leading-snug break-words pr-2 ${
                      opt.isCorrect
                        ? "text-black"
                        : "text-gray-500"
                    }`}
