...
import type { AuthResponse, FieldErrors } from "../types/quiz";
...
export default function AuthCard({ setToken }: AuthCardProps) {
  ...
  const handleAuth = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const endpoint: "login" | "signup" = isLogin ? "login" : "signup";
      ...
      <div className="w-full md:w-7/12 bg-white p-10 md:p-16 flex flex-col justify-center items-center">
        ...
        <form onSubmit={handleAuth} className="flex flex-col gap-4 w-full max-w-sm">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full mt-4 py-4 rounded-full bg-[#72D177] text-white font-black uppercase tracking-widest text-sm hover:bg-[#5eb663] hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
          >
